...
internal sealed class MemoryImpressionGenerator
{
    ...
    private async Task GenerateAsync(string requestId, string display, string system, string user, int timeoutSeconds)
    {
        ...
        try
        {
            ...
            ModEntry.SMonitor?.Log($"Memory impression compressed for {display} ({impression.Length} chars).", LogLevel.Info);
        }
        catch (Exception ex)
        {
    ...
    private void Fail(string requestId, string display, string reason)
    {
        ...
        ModEntry.SMonitor?.Log($"Memory impression compression failed for {display}: {reason}; memories stay queued for a later retry.", LogLevel.Info);
    }
