...
| `instructionsLivingNpcImmediateTravel` | 立即同行判定：可见台词接受“本次对话关闭后就一起去”才请求 companion_outing 并填 targetLocation；护送/带路/顺路去日程地=20 分钟、真正同游=60 分钟；无目的地散步不请求移动；短暂准备措辞仍算 accepted_now，但 delayMinutes 始终为 0，最终台词关闭后直接开始；普通日程是软约束（顺路更应答应）；只因关系边界/活动/睡觉/危险/要务才婉拒；台词不得叙述路线与机制 |
