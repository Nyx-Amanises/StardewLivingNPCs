...
add("instructionsLivingNpcImmediateTravel", "Request companion_outing only when the visible line accepts going together when this dialogue closes. Fill targetLocation with a supported enum. Escorting, guiding, or walking to a schedule place is 20 minutes; a real outing is 60 minutes. Brief preparation wording still counts as accepted_now, but always keep delayMinutes at 0 because LivingNPCs starts the outing after the final line is dismissed. A normal schedule is a soft constraint; decline only for boundaries, events, sleep, danger, or urgent duties. Do not narrate route mechanics.", "只有可见台词接受‘本次对话结束后就一起去’时才请求companion_outing。targetLocation使用支持的枚举。护送、带路或顺路去日程地点为20分钟；真正同游为60分钟。短暂准备措辞仍算accepted_now，但delayMinutes必须始终为0，因为LivingNPCs会在最终台词关闭后直接开始出游。普通日程是软约束；只有关系边界、活动、睡眠、危险或紧急职责才应拒绝。不要叙述路线机制。");
add("instructionsLivingNpcImmediateTravelOptimized", "Only accepted when this dialogue closes => companion_outing. Escort/guide/schedule walk 20 minutes; true outing 60. Brief preparation still counts as now, but delayMinutes is always 0. Decline only for strong reasons. No route mechanics in visible text.", "只有接受在本次对话关闭后出发才用companion_outing。护送/带路/顺路日程为20分钟；真正同游60分钟。短暂准备仍算现在，但delayMinutes始终为0。只有强理由才拒绝。可见文本不写路线机制。");
add("instructionsLivingNpcTravelConsent", "Set travelConsent to accepted_now, accepted_later, declined, tentative, or none. accepted_now means departure when this dialogue closes; brief preparation wording still counts as now, but delayMinutes remains 0. accepted_later means later or another day. declined means no or not now. tentative means unclear interest. none means travel was not discussed. Do not request companion_outing unless travelConsent is accepted_now.", "travelConsent只能为accepted_now, accepted_later, declined, tentative, none。accepted_now表示本次对话关闭后出发；短暂准备措辞仍算现在，但delayMinutes保持为0。accepted_later表示晚点或改天；declined表示拒绝或现在不行；tentative表示态度含糊；none表示未讨论出行。除accepted_now外不得请求companion_outing。");
add("instructionsLivingNpcTravelConsentOptimized", "travelConsent enum: accepted_now, accepted_later, declined, tentative, none. Only accepted_now may create companion_outing; enum values stay English.", "travelConsent枚举：accepted_now, accepted_later, declined, tentative, none。只有accepted_now可以创建companion_outing；枚举值保持英文。");
add("instructionsLivingNpcHelpRequests", "Create helpRequests only when context says this NPC may ask today and the visible line actually asks. Only type item_request is allowed. requestedItemId must come from the current reasonable-item list. dueInDays must be 1-7. Do not ask for delivery errands, letters, schedule changes, trivia, or non-item favors. New requests usually require requiresAcceptance=true; use false only when the farmer offered and the NPC accepted. Each request may have up to 3 item steps. followUpPotential is none or deeper_relationship. Use helpRequestUpdates for accepted, declined, advanced, or fulfilled when the farmer response or hand-in clearly changes an existing request.", "只有上下文说明该NPC今天可以开口求助，且可见台词真的提出请求时，才创建helpRequests。只允许type为item_request。requestedItemId必须来自当前合理物品列表。dueInDays为1-7。不得请求跑腿、送信、改日程、知识问答或非物品帮忙。新请求通常requiresAcceptance=true；只有农夫主动提出帮忙且NPC接受时才用false。每个请求最多3个物品步骤。followUpPotential为none或deeper_relationship。当农夫回应或交付明确改变现有请求时，用helpRequestUpdates记录accepted、declined、advanced或fulfilled。");
