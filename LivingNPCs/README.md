...
- 读取当前场景，例如时间、天气、地点、附近 NPC 和玩家与 NPC 的关系，并把这些因素用于行为选择和 ValleyTalk 上下文。
...
LivingNPCs 会读取当前世界场景。清晨、深夜、雨雪、室内外、酒馆/商店/农场/矿区、附近是否有其他 NPC，以及玩家与 NPC 的好感心数都会形成一个轻量场景偏移。它不会强迫 NPC 做某个动作，只会影响 `Auto` 行为选择概率，并作为 ValleyTalk 的安静上下文。

当手动行为模式设为 `Auto` 时，NPC 状态会反过来影响行为选择。注意度和回应倾向越高，NPC 越可能走近或明显回应；心情更外露时，更容易显示表情。固定测试模式仍然按你选择的行为执行，方便验证单个动作。

当同一个 NPC 与你逐渐熟悉后，`Auto` 模式下更容易出现走近、转向或自然回应。查看记忆时也会显示“对玩家熟悉度”“行为倾向”和“当前场景”，ValleyTalk prompt 里会收到对应上下文。
