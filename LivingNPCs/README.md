...
- 为 Stardew Valley Expanded 和 Ridgeside Village 的部分主要 NPC 提供更细的背景提示；未手写覆盖的角色会自动回落到 `Data/Characters` 推断。
...
对自定义 NPC，LivingNPCs 会优先查找内置角色资料；如果没有，就从游戏已经加载的 `Data/Characters` 推断基础性格。这样 SVE、RSV 和其他内容包 NPC 不会全部落到随机默认性格上。SVE 和 RSV 的主要角色还会带一小段背景摘要，例如工作、家庭、村庄角色或明显的性格压力点，让 ValleyTalk 更容易把他们说得像自己。
