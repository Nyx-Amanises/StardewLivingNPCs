...
internal static class HelpRequestAdvisor
{
    ...
    public static string BuildPromptLabel(NPC npc, WorldProgressSnapshot progression)
    {
        ...
        return $"theme {profile.Theme}; currently reasonable item requests: {itemText}; allowed help request type: item_request only; never create question_request; request relationship tier: {relationshipText}; request depth: {stageText}; world-stage constraint: {routeText}{chainText}; whenever the visible reply has this NPC ask the farmer to bring or find listed items — whether the farmer offered first or the NPC raised it — you MUST include exactly one hidden helpRequests entry for the whole favor and never leave the favor only in spoken text. For a one-step request, the visible reply may explicitly request only that entry's single requestedItemId/requestedItemLabel. If the visible reply explicitly requests multiple items, encode every requested item as ordered steps in that same entry, in the exact spoken order; never split them across entries, omit one, or reorder them. Never add an unencoded bonus or optional item with wording such as 'if you can also bring', 'while you're at it', 'another would be better', or 'that would make it perfect'; either encode every required item as a step or do not mention it as part of the request. When the farmer agrees to such a favor, include a hidden helpRequestUpdates entry with status accepted; if no listed item naturally fits, keep the visible reply as ordinary conversation and do not open a hidden request.";
    }
        ...
        WorldProgressSnapshot progression)
    {
        ...
        return $"; optional two-step chain suggestion (use only when the conversation genuinely supports one bigger favor): step 1 = {chain.Value.First.Label} {chain.Value.First.ItemId}, then step 2 = {chain.Value.Second.Label} {chain.Value.Second.ItemId}; if used, the visible reply must clearly request both as required steps in this order, and one hidden helpRequests entry must encode both steps in the same order. Otherwise request only one item. Never turn step 2 into an unencoded optional add-on such as 'if you can', 'while you're at it', 'another would be better', or 'that would make it perfect'";
    }
