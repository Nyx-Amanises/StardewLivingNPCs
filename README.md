...
- **自由输入 AI 对话：**默认按住 <code>LeftAlt</code>，再点击 NPC 或使用交互键。回复默认**边生成边逐字显示**（流式），生成中按 <code>Esc</code> 可取消；如需回到"思考中……等完整回复"的经典对话框，可在 GMCM 关闭「流式显示回复」。
- **普通对话：**默认直接右键仍显示原版台词。
...
| 流式显示回复 | 默认开启；自由输入对话的回复边生成边逐字显示，关闭后等完整回复再显示 |
...
- <code>LeftShift + J</code>：把附近 NPC 的状态与记忆摘要输出到 SMAPI 控制台。

输入对话热键与查看记忆快捷键可以在 GMCM 中调整；行为测试键需在 <code>config.json</code> 的 <code>BehaviorHotkey</code> 中修改。
