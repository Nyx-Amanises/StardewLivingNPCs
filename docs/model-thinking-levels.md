...
礼物信的请求超时由 `config.json` 中的 `AiGiftMailTimeoutSeconds` 控制，新默认值为 **85 秒**，有效范围为 **5–180 秒**。已有用户显式设置值保留，仅在超出范围时限幅，不会统一将旧的 30 秒配置改为 85 秒。取得并发名额后开始计算请求预算；外层取消和通过 `LlmRequest.TimeoutOverride` 传入 HTTP 层的超时使用同一预算，避免被普通对话的 `QueryTimeout` 提前截断。成功和失败日志分别记录请求耗时、实际超时上限及等待并发名额的排队耗时。

记忆印象的 `MemoryImpressionTimeoutSeconds` 默认仍为 **45 秒**，本轮未改变其超时行为。10,000 token 控制输出量，不会自动延长请求等待时限。

## 配置和请求

设置菜单只保留一个“思考档位”，配置键为 `ThinkingLevel`，默认 Auto。对话、动作判定、元数据提取、记忆印象、礼物信和可选的微行为规划共用这一设置。额外的 AI 语义上下文路由已于 2026-09-13 移除。

下拉框固定提供 Auto、Off、Low、Medium、High、XHigh、Max、Ultra，移除 Minimal（最少）。切换提供商或模型名、保存和重新打开菜单都会保留所选档位，模型名为空、暂时拼错或使用自定义别名也不会把选项缩为 Auto。菜单规范大小写和首尾空白，旧 Minimal 转为 Low；模型支持范围的转换发生在请求发送时，不回写配置。

旧配置自动迁移：优先沿用有效的 `ChatThinkingLevel`，聊天值缺失、为空或无效时再取 `RoutingThinkingLevel`，都不可用则使用 Auto。显式 Auto 也算有效选择。新 `ThinkingLevel` 字段优先于两个旧字段，不受 JSON 字段顺序影响；保存后仅保留新字段。恢复默认设为 Auto。

GMCM 的选项在注册时固定，编辑值在保存时逐项提交，因此不能根据正在编辑的模型名过滤下拉框。仅更换模型名不再重建菜单；更换提供商仍会刷新对应的连接字段。旧版已经存成 Auto 的档位无法推断原选择，需要在修复版中重新选择并保存。

`LlmRequest.OutputFormat` 单独表示纯文本或 JSON。邮件和记忆使用纯文本；动作判定和元数据提取明确请求 JSON。选择 Off 不再把纯文本任务自动变成 JSON。请求重试克隆保留输出格式和 token 上限。

请求中的历史字段 `DisableThinking` 只保留辅助任务的传输选择语义，不再指定另一套思考力度。火山引擎已识别的混合模型把统一 Off 转为 disabled、其他显式档位转为 enabled；Auto 和不能切换的模型省略该参数。

## 模型能力

Auto 表示省略思考控制参数、使用服务端默认值。下表列出实际请求可用档位并省略 Auto；Off 表示模型可以关闭思考。菜单选项不随此表过滤。

部分模型协议仍有原生 Minimal，例如 Gemini 的最低档适配；它只作为内部参数保留，不再作为配置或菜单档位提供。
