...
2026-09-12，基于当时工作区的静态阅读。本文只评估，不接入新展示路径；未构建、运行游戏、读取用户存档或发送线上请求。引用中的方法名用于定位，行号可能随后续修改移动。

2026-09-13 更新：额外 AI 语义上下文路由已移除，下面的当前流程和诊断阶段说明已同步。提前展示仍是待实施的架构评估。
...
当前路径可概括为：请求快照与本地上下文装配 → 主请求（首个正文之前的等待、正文、选项、隐藏 JSON）→ 元数据与最终修订 → 主线程展示 → 一次提交。提前展示只能重叠其中一部分等待与玩家阅读时间，不会让模型尚未生成的正文提前出现。
...
1. **先记录各阶段等待，保持现有 UI。** 现有 [LlmTransportTiming](../LivingNPCs/Dialogue/Llm/LlmTransportTiming.cs) 已记录 headers、first-content、complete；`DialogueEngine.LogDiagnostics` 已区分 main、metadata、action。未来增加正文边界到达、首字实际绘制、最终安装/提交时间，并关联 generation/attempt。first-content 可能只是 `-`，不是玩家真正看到字的时间。用可控假客户端先验证测量口径，再在获准的运行验证中观察实际收益；不预估“能快几秒”。
